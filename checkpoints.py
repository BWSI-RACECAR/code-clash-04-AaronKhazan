...
class Solution:    
    def longestdistance(self, checkpoints):
            ...
            for i in range(len(checkpoints)):
                min_index = i
                for k in range(i+1, len(checkpoints)):
                    if checkpoints[min_index] > checkpoints[k]:
                        min_index = k

                checkpoints[i], checkpoints[min_index] = checkpoints[min_index], checkpoints[i]

            long_dist = 0
            for s in range(len(checkpoints)-1):

                if long_dist < (checkpoints[s+1] - checkpoints[s]):
                    long_dist = checkpoints[s+1] - checkpoints[s]
            return(long_dist)
